...
class Move_Robot_Action:
    ...
    #Definimos el constructor de la clase Move_Robot_Action:
    ...
    def __init__(self, name):
        ...
        self.Posicion_x_1=0  
        self.Posicion_x_2=0
        self.distancia_recorrida_x=0


        self.Posicion_y_1=0
        self.Posicion_y_2=0
        self.distancia_recorrida_y=0
    ...
    def get_position(self,msg: Odometry):
        ...
        #En este caso no es necesario definir las variables de posición como variables globales.
        #Como las hemos definido como variables de clase, los métodos de la clase tienen acceso a
        #a estas.
        ...
        self.Posicion=msg.pose.pose.position

        self.Posicion_x=self.Posicion.x
        self.Posicion_y=self.Posicion.y
        ...
        if goal.Movimiento==1: #Movimiento lineal
           ...
           # Distancia a avanzar (en metros). Esta se obtiene a partir de la solicitud que hace el cliente (goal)
            ...
            while not rospy.is_shutdown():
                ...
                if self.Posicion_x!=0 and self.Posicion_y!=0 and self.yaw!=0:   #Posicion_x se inicializa en cero. Este valor debe cambiar cuando el robot empieza a moverse
                    ...
                    if Contador==0:
                        ...
                        x_inicial=self.Posicion_x
                        print("Posición inicial en x: " +str(x_inicial))
                        y_inicial=self.Posicion_y
                        ...
                        self.Posicion_x_1=x_inicial
                        self.Posicion_y_1=y_inicial
                        ...
                        distancia_recorrida=distancia_recorrida+math.sqrt(math.pow(self.distancia_recorrida_x,2)+math.pow(self.distancia_recorrida_y,2))
                        ...
                        print("Posición en x: " +str(self.Posicion_x))
                        print("Posición en y: " +str(self.Posicion_y))
                        ...
                        if distancia_recorrida>distance_to_move:
                            ...
                            self._feedback.Feedback[0]=self.Posicion_x
                            self._feedback.Feedback[1]=self.Posicion_y
                            self._feedback.Feedback[2]=self.yaw
            ...
            #En este caso queremos girar el robot un número de grados determinado por el usuario, no especificar directamente la
            #posición que debe alcanzar el robot. Para ello, debemos tomar la orientación inicial del robot.

            yaw_inicial=self.yaw

            #El target_rad será la suma de la orientación inicial del robot más el ángulo especificado por el usuario

            target_rad = yaw_inicial+goal.Giro*math.pi/180  #Al ángulo medido por el robot queremos añadirle una cantidad de ángulos especificada por el usuario

            #Esto es por si el target_rad supera los 2pi radianes

            #if target_rad>2*math.pi:
            ...
            while abs(target_rad-self.yaw)>self.kp:

                #yaw no está definido de 0 a 2pi radianes, sino que está definido entre 0,pi y -pi,0
                #Para corregir esto, debemos sumar un ángulo de 2pi cuando el valor de yaw se encuentra
                #por debajo de 0.
                ...
                #Publicamos el mensaje de velocidad en el tópico 'cmd_vel'
                pub.publish(twist)
                rospy.loginfo(f'target: {target_rad}, current: {self.yaw}')
                ...
                if abs(target_rad-self.yaw)<=self.kp:
